...
def extract_xexp(package_path):
    ...
    # The XEX delta is by far the largest file; the rest are .str string tables.
    target = max(files, key=lambda e: e["length"])
    data = stfs.read_file(target)
    if data[:4] != b"XEX2":
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="Extract/select the title-update XEX delta patch from STFS package(s).")
